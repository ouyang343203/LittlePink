...
//        let privacyInfo : NSMutableAttributedString = NSMutableAttributedString.init(string: "\n亲，感谢您对XXX一直以来的信任！我们依据最新的监管要求更新了XXX《隐私权政策》，特向您说明如下\n1.为向您提供交易相关基本功能，我们会收集、使用必要的信息；\n2.基于您的明示授权，我们可能会获取您的位置（为您提供附近的商品、店铺及优惠资讯等）等信息，您有权拒绝或取消授权；\n3.我们会采取业界先进的安全措施保护您的信息安全；\n4.未经您同意，我们不会从第三方处获取、共享或向提供您的信息；", attributes: [NSAttributedStringKey.paragraphStyle:paragraphStyle])
        let privacyInfo : NSMutableAttributedString = NSMutableAttributedString.init(string: "\n亲，感谢您对XXX一直以来的信任！我们依据最新的监管要求更新了XXX《隐私权政策》，特向您说明如下\n1.为向您提供交易相关基本功能，我们会收集、使用必要的信息；\n2.基于您的明示授权，我们可能会获取您的位置（为您提供附近的商品、店铺及优惠资讯等）等信息，您有权拒绝或取消授权；\n3.我们会采取业界先进的安全措施保护您的信息安全；\n4.未经您同意，我们不会从第三方处获取、共享或向提供您的信息；", attributes: [NSMutableAttributedString.Key.paragraphStyle:paragraphStyle])
        
        privacyInfo.setAttributes([NSMutableAttributedString.Key.foregroundColor:UIColor.blue], range: privacyInfo.mutableString.range(of: "《隐私权政策》"))
        
        let privacyInfoController : UIAlertController = UIAlertController.init(title: "温馨提示(隐私合规示例)", message: "", preferredStyle: .alert)
        
        privacyInfoController.setValue(privacyInfo, forKey: "attributedMessage")
        
        let agreeAllAction : UIAlertAction = UIAlertAction.init(title: "同意(下次不提示)", style: .default) { UIAlertAction in
        ...
        let agreeAction : UIAlertAction = UIAlertAction.init(title: "同意", style:.default) { UIAlertAction in
            //更新用户授权高德SDK隐私协议状态. since 8.1.0
            AMapLocationManager.updatePrivacyAgree(AMapPrivacyAgreeStatus.didAgree)
        }
        ...
        let notAgreeAction : UIAlertAction = UIAlertAction.init(title: "不同意", style:.default) { UIAlertAction in
        ...
        appDelegate.window?.rootViewController?.present(privacyInfoController, animated: true, completion: {
